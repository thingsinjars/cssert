...
(function () {
    ...
    createTestUnit = function createTestUnit(target, chosenStyles) {
        var returnable = {}, unit = generateUnit($(target).get()[0]), selector = target.tagName, $target = $(target), chain = $target.parents();
    ...
    jQs.onload = function () {
        ...
        $('*').not('.cssert-modal').hover(function(e) {
          ...
          $t.css({'outline' : '2px solid white'});
          $t.css({'box-shadow' : '0 0 10px black'});
          e.preventDefault();
          e.stopPropagation();
        }, function(e) {
          var $t = $(e.target);
          $t.css({'outline' : $t.data('originalOutline')});
          $t.css({'box-shadow' : $t.data('originalShadow')});
          e.preventDefault();
          e.stopPropagation();
        });
